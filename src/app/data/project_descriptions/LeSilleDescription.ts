const LeSilleDescription = `
    <div class="text-xs flex mb-[95px] gap-[75px]">
        <ul>
            <li>Card Game</li>
            <li>Educative</li> 
            <li>Illustration</li>
            <li>Packaging</li>
            <li>History of Interiorism</li>
        </ul>
        <ul class="text-low_opac">
            <li>Procreate</li>
            <li>Illustrator</li>
        </ul>
    </div>
    <div class="text-xs mb-12">
        <span class="block font-bold mb-[10px]">ESP</span>
        <p class="mb-4">
            El proyecto se centra en profundizar sobre la definición del diseño escandinavo, los <br>
            representantes más relevantes y las piezas más emblemáticas.
        </p>
        <p class="ml-[75px] mb-4">
            El juego de cartas integra de manera lúdica <br>
            del diseño de mobiliario.
        </p>
        <p class="mb-4">
            La propuesta fue centrar el juego en 6 diseñadores, a cada uno de estos le <br>
            corresponden 4 sillas. El juego consiste en coleccionar las sillas del diseñador que se <br>
            te ha otorgado. Eso, lo consigues mediante unas cartas de pistas, 4 pistas por silla.
        </p>
        <div class="ml-[75px] mb-4">
            <span>6 cartas de diseñador</span> / <span>24 cartas de sillas</span> / <span>96 cartas de pistas</span>
        </div>
        <p class="ml-[31px] text-right">
            Los colores fueron extraídos de paletas relacionadas con diseño escandinavo <br>
            añadiéndole saturación para extrapolarlo al formato juego. Utilizando también la <br>
            simplicidad de este estilo para la estética de las cartas.
        </p>
    </div>
    <div class="text-xs text-right text-low_opac">
        <span class="block font-bold mb-[10px]">ENG</span>
        <p class="mb-4">
            The project focuses on delving into the definition of Scandinavian design, its most re- <br>
            levant representatives, and the most iconic pieces.
        </p>
        <p class="mr-[75px] mb-4">
            The card game playfully incorporates content related to <br>
            furniture design.
        </p>
        <p class="mb-4">
            The proposal was to focus the game on 6 designers, each associated with 4 chairs. <br>
            The objective of the game is to collect the chairs of the designer you have been <br>
            assigned. You achieve this by using clue cards, with 4 clues per chair.
        </p>
        <div class="mr-[35px] mb-4">
            <span>6 designer cards</span> / <span>24 chair cards</span> / <span>96 clue cards</span>
        </div>
        <p class="ml-[31px]">
            The colors were extracted from palettes related to Scandinavian design, adding <br>
            saturation to adapt them to the game format. The simplicity of this style was also used <br>
            for the aesthetics of the cards.z
        </p>
    </div>
`

export default LeSilleDescription